...
TEST_CASE("AStar basique") {
    Graphe<Chemin, Case> g;
    auto s1 = g.creeSommet(Case(Position<>(0,0), nullptr));
    auto s2 = g.creeSommet(Case(Position<>(1,1), nullptr));
    g.creeArete(Chemin(), s1, s2);

    AStarFunctions::destination = s2;
    auto dest = AStarT<Graphe<Chemin, Case>, Sommet<Case>>::aStar(g, s1, &AStarFunctions::hh);
...
TEST_CASE("AStar complexe") {
    Graphe<Chemin, Case> g;
    auto s1 = g.creeSommet(Case(Position<>(0,0), nullptr));
    auto s2 = g.creeSommet(Case(Position<>(2,0), nullptr));
    auto s3 = g.creeSommet(Case(Position<>(0,1), nullptr));
    auto s4 = g.creeSommet(Case(Position<>(1,1), nullptr));
    auto s5 = g.creeSommet(Case(Position<>(2,1), nullptr));
    g.creeArete(Chemin(), s1, s2);
